    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top">
      ...
      <div class="floater">
        <ul class="navbar-nav mr-auto left">
          ...
          <li class="nav-item">
            <router-link :to="{ name: 'Venues' }" class="nav-link"
              >Venues</router-link
            >
          </li>
          <li class="nav-item">
        ...
        <ul class="navbar-nav mr-auto right">
          <div v-if="isAuth">
            <li class="nav-item">
              <span class="navbar-text" style="display: inline-flex"
                >Welcome,&nbsp;
                <a v-on:click="MyProfile()" class="profile-link">{{
                  username
                }}</a>
              </span>
            </li>
...
export default {
  ...
  updated() {
    if (this.$cookies.isKey("session")) {
      this.isAuth = true;
      this.$http
        .get(url + "/users/" + this.$cookies.get("session").userId)
        .then(
          function (response) {
  ...
  methods: {
    Logout: function () {
      ...
      this.$http
        .post(url + "/users/logout", JSON.stringify({}), { headers })
        .then(
          function (response) {
