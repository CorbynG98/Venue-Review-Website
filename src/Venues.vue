<template>
  <div>
    <b-container fluid>
      <div
        v-bind:class="[filtersShowing ? 'showing' : 'hiding']"
        style="transition: display 4s ease 4s; overflow: hidden"
      >
        <b-row>
            ...
            <b-form-group label-cols-sm="3" label="Search Name" class="mb-0">
              <b-input-group>
                <b-form-input
                  ...
                  v-on:input="filterCities"
                ></b-form-input>
                <b-input-group-append>
                  <b-button :disabled="!filter" v-on:click="emptyFilter()"
                    >Clear</b-button
                  >
                </b-input-group-append>
              </b-input-group>
      ...
      <div style="width: 100%; margin-top: 1rem">
        <div>
          ...
          <div>
            <div v-if="mineShowing">
              <b-button
                size="lg"
                variant="success"
                v-on:click="showAllVenues()"
                ref="btnShow"
                style="float: left; margin-bottom: 1rem; margin-left: 1rem"
                >All Venues</b-button
              >
            </div>
            <div v-else>
          ...
          <b-modal id="newVenueModal" size="xl" title="New Venue" centered>
            <form ref="form">
              <div
                style="
                  display: inline-flex;
              ...
              >
                <b-form-input
                  v-model="newVenueModel.venueName"
                  placeholder="Venues name"
                  style="margin-right: 1rem"
                ></b-form-input>
                <b-form-select
                  v-model="newVenueModel.categoryId"
                  :options="categoryOptions"
                  style="margin-right: 1rem; margin-left: 1rem"
                ></b-form-select>
                <b-form-input
                  v-model="newVenueModel.city"
                  placeholder="City"
              ...
              <div style="margin-bottom: 1rem">
                <b-form-input
                  v-model="newVenueModel.shortDescription"
                  placeholder="Short Description"
                  style="margin-bottom: 1rem"
                ></b-form-input>
                <b-form-input
                  v-model="newVenueModel.longDescription"
                  placeholder="Long Description"
                ></b-form-input>
              </div>
              <b-form-input
                v-model="newVenueModel.address"
                placeholder="Address"
                style="margin-bottom: 1rem"
              ></b-form-input>
              <div style="display: inline-flex; margin-bottom: 0; width: 100%">
                <b-form-input
                  v-model="newVenueModel.longitude"
                  placeholder="Latitude"
                  style="margin-right: 1rem"
                ></b-form-input>
                <b-form-input
                  v-model="newVenueModel.latitude"
      ...
      >
        ...
        <template slot="primaryPhoto" slot-scope="row">
          <b-img
            v-bind:src="getLink(row.item.venueId, row.item.primaryPhoto)"
            fluid
            style="
              max-width: 200px;
              max-height: 200px;
          ...
          <b-button size="sm">
            <router-link
              :to="{
                name: 'Venue',
                params: {
                  venueId: row.item.venueId,
                  meanStarRating: row.item.meanStarRating,
                  modeCostRating: row.item.modeCostRating,
                },
              }"
              class="link"
              >Venue Details</router-link
            >
          </b-button>
        </template>
      </b-table>
...
export default {
  ...
  methods: {
    getVenues: function () {
      ...
      this.isBusy = true;
      this.$http.get(url + "/venues", { params: sortData }).then(
        function (response) {
          this.items = response.body;
          for (let venue in this.items) {
            citySet.add(this.items[venue].city);
            if (
              this.items[venue].meanStarRating == 0 ||
              this.items[venue].meanStarRating == null
            )
              this.items[venue].meanStarRating = 3;
            for (let cat in this.categories) {
              if (
                parseInt(cat) + 1 ===
                parseInt(this.items[venue].categoryId)
              ) {
    ...
    getLink: function (venueId, photoFilename) {
      if (venueId == null || photoFilename == null)
        return "https://storage.googleapis.com/public-s365-venue-review/default.png";
    ...
    uploadNewVenue: function () {
      ...
      this.$http.post(url + "/venues", this.newVenueModel, { headers }).then(
        function (response) {
          this.filterCities();
          this.$root.$emit("bv::hide::modal", "newVenueModal");
        },
    ...
    runSortCode: function (queryParams) {
      ...
      this.$http.get(url + "/venues", { params: queryParams }).then(
        function (response) {
          this.items = response.body;
          for (let venue in this.items) {
            if (
